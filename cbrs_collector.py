#!/usr/bin/env python3
"""
Incremental CBRS perp candle collector for Hyperliquid.
...
5000 candles PER INTERVAL. That is ~3.5 days at 1m, ~104 days at 30m, ~208
days at 1h. Anything older is unrecoverable, so you must poll regularly and
append into your own archive.

Why several intervals: xyz:CBRS launched 2026-05-01 as a pre-IPO perp, 13 days
before the 2026-05-14 Nasdaq listing. That pre-IPO window is already outside
the 1m and 15m recency horizons -- only 30m and coarser still reach it, and
the 30m horizon crosses inception around 2026-08-13. Collecting 1m + 30m +
1h + 1d keeps the fine-grained live data AND locks the pre-IPO history into
the archive before it ages out.

MODES
    python cbrs_collector.py                      # one-shot: fetch new candles, append, exit
    python cbrs_collector.py --loop               # run forever, poll every --poll-sec (default 60)
    python cbrs_collector.py --coins xyz:CBRS     # skip discovery (also auto-cached after first run)
    python cbrs_collector.py --intervals 1m,1h    # override the interval set
...
def slug(coin):
    """xyz:CBRS -> xyz_CBRS (':' is not a legal filename char on Windows)."""
...
def main():
    ...
    ap.add_argument("--intervals", default=",".join(INTERVALS),
                    help="comma-separated, e.g. 1m,30m,1h,1d")
