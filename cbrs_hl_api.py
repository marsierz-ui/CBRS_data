#!/usr/bin/env python3
"""
CBRS perp candles from the Hyperliquid public info API (one-shot pull).
...
per interval, so at 1m you get ~3.5 days of history, at 30m ~104 days, at 1h
~208 days. For the pre-IPO window (2026-05-01 .. 2026-05-14) use 30m or
coarser; the fine intervals no longer reach back that far.

HIP-3 builder-deployed perps are named {dex}:{coin} (e.g. "xyz:CBRS"). The dex
...
USAGE
    python cbrs_hl_api.py --interval 1h --start 2026-05-01
    python cbrs_hl_api.py --coin xyz:CBRS --interval 1m
    python cbrs_hl_api.py --interval 1h --stock data/1min/CBRS_2026-05-14.csv \
...
IPO_DATE = "2026-05-14"        # Nasdaq listing; market ran pre-IPO from 2026-05-01
...
def merge_and_chart(perp, stock, tol_sec, outdir):
    ...
    try:
        ...
        ax.plot(perp_close.ts, perp_close.perp_close, lw=0.8, label="Perp CBRS")
        ax.plot(stock.ts, stock.stock_close, lw=1.1, label="Nasdaq CBRS")
        ax.axvline(pd.Timestamp(IPO_DATE, tz="UTC"), ls="--", lw=1, color="grey")
        ax.set_title("CBRS: pre-IPO perp vs listed stock (dashed line = IPO)")
...
def main():
    ...
    ap.add_argument("--interval", default="1h", choices=sorted(INTERVAL_VALID))
    ap.add_argument("--symbol", default=SYMBOL, help="symbol to discover (default CBRS)")
    ap.add_argument("--coin", default=None, help="exact {dex}:{coin} id; skips discovery")
    ap.add_argument("--start", default="2026-05-01", help="UTC YYYY-MM-DD (market inception)")
